...
def process_video():
    ...
    for i, image in enumerate(reader):
        ...
        # image = cv2.merge([r,g,b])[100:580, 100:740, :]
                    ...
                    if i>15:
                        if q > 10:
                            if det_type == 0:  # 0 == face;
                                ...
                                if showFaceDet:
                                    cv2.rectangle(pixs, (col-scale/2, row-scale/2), (col+scale/2, row+scale/2), (0, 0, 255), 2)
                            elif det_type == 1:  # 1 == pupil;
                                if showPupil:
                                    cv2.circle(pixs, (int(col), int(row)), 4, (0, 0, 255), -1, 8, 0)
                            elif det_type == 2:  # 2 == facial landmark;
                                if showLandmarkPoints:
                                    cv2.circle(pixs, (int(col), int(row)), 4, (0, 255, 0), -1, 8, 0)
                            elif det_type == 3:
